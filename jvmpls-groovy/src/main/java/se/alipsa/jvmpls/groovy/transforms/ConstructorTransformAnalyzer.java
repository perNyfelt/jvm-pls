...
final class ConstructorTransformAnalyzer implements TransformAnalyzer {
  ...
  public List<SyntheticMemberSpec> analyze(
    ...
    return List.of(
        SyntheticMemberSpec.syntheticConstructor(
            context.ownerFqn(),
            new MethodSignature(
                parameterTypes,
                VoidType.INSTANCE,
  ...
  private static boolean sameSignature(
      List<JvmType> propertyTypes, ConstructorNode constructor, TransformContext context) {
    ...
    for (int i = 0; i < constructor.getParameters().length; i++) {
      if (!propertyTypes
          .get(i)
          .equals(context.typeOf().apply(constructor.getParameters()[i].getType()))) {
        return false;
      }
    }
    return true;
