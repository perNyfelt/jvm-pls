...
/**
 * Service-provider interface for build tool integrations that can detect a workspace, resolve its
 * build model, and expose the resulting classpath/JDK information to the server.
 */
public interface BuildToolPlugin {
  ...
  /**
   * Returns the plugin priority used when several build tools apply to the same workspace. Higher
   * values win.
   */
   ...
   *
   * @throws BuildResolutionException if the workspace matches this plugin but the build cannot be
   *     resolved
   */
