...
public interface JvmLangPlugin {
  ...
  default double claim(String fileUri, Supplier<CharSequence> contentPreview) {
    String ext =
        fileUri.contains(".") ? fileUri.substring(fileUri.lastIndexOf('.') + 1).toLowerCase() : "";
  ...
  default SymbolInfo resolveSymbol(String fileUri, String symbolName, CoreQuery core) {
    return null;
  }

  /** Attempt to resolve a symbol name at a specific position in file context to a known symbol. */
  default SymbolInfo resolveSymbol(
